...
class FinancialReportsTable
{
    public static function configure(Table $table): Table
    {
        return $table->columns([
            ...
            TextColumn::make('period_a_title')->label(__('financial_reports.current_period'))->searchable()->alignment(Alignment::Center),
            TextColumn::make('period_b_title')->label(__('financial_reports.base_period'))->searchable()->alignment(Alignment::Center),
            TextColumn::make('net_profit_growth')->label(__('financial_reports.net_profit_growth'))->suffix('%')->color(fn ($state) => $state >= 0 ? 'success' : 'danger')->alignment(Alignment::Center),
            TextColumn::make('eps_growth')->label(__('financial_reports.eps_growth'))->suffix('%')->color(fn ($state) => $state >= 0 ? 'success' : 'danger')->alignment(Alignment::Center),
        ])->filters([
